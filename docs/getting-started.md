...
## Will Ongakuvault allow me to stream my archived audio files?
...
## Does OngakuVault support multiple artists / genres in file metadata?
Yes! OngakuVault supports storing multiple artists (and genres) in a single file's metadata (only if the metadata library [ATL](https://github.com/Zeugma440/atldotnet) and the metadata container support it) using a separator character (default: ``;``). For example: "``Artist1; Artist2``", where the primary value, or artist, would be **Artist1**.
