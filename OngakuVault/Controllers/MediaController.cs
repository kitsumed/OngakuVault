	...
	public class MediaController : ControllerBase
	{
		...
		public async Task<ActionResult> GetMediaInfo(string mediaUrl, CancellationToken cancellationToken)
		{
			...
			catch (ProcessedScraperErrorOutputException ex)
			{
				...
				if (ex.IsKnownError)
				{
					_logger.LogWarning("Known scraper error was returned by the scraper when fetching media information on : '{mediaUrl}'. Error: {message}.\nOriginal Scraper Error: {originalError}", mediaUrl, ex.Message, ex.OriginalError);
					return StatusCode(StatusCodes.Status500InternalServerError, ex.Message);
				}
				else _logger.LogError("An unexpected scraper error occurred while fetching media information on : '{mediaUrl}'.\nScraper outputs: --> {message}", mediaUrl, ex.Message);
			}
